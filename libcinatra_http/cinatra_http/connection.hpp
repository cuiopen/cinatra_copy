...
namespace cinatra
{
	...
	{
	public:
		explicit connection(ASIO_NS::io_service& io_service, request_handler_t& handler,
		...
		explicit connection(ASIO_NS::io_service& io_service, request_handler_t& handler,
			ASIO_NS::ssl::context& ctx, std::size_t max_req_size, long keep_alive_timeout)
		...
		void start()
		{
			...
			reply_.set_get_connection_func([weak_self, this](bool kill_timer)
			{
				...
				return std::make_shared<response::connection>(reply_,
					...
					[self, this](std::vector<ASIO_NS::const_buffer> const& buffers, response::handler_ec_size_t handler) {delay_write(buffers, std::move(handler));},
		...
		void reset_timer()
		{
			...
			deadline_.async_wait([weak_self](error_code const& ec)
			{
				auto self = weak_self.lock();
	...
	private:
		void do_close(ASIO_NS::ip::tcp::socket const&)
		{
			error_code ec;
			socket_.close(ec);
		}
#if !DISABLE_ASIO_OPENSSL
		void do_close(ASIO_NS::ssl::stream<ASIO_NS::ip::tcp::socket> const&)
		{
			error_code ec;
		...
		void do_read()
		{
			...
			socket_.async_read_some(ASIO_NS::buffer(buf.curr_ptr(), buf.remain_size()),
		...
		void do_read_body()
		{
			...
			ASIO_NS::async_read(socket_, ASIO_NS::buffer(buf.curr_ptr(), req_len - buf.size),
				[self, this](error_code ec, std::size_t length)
			{
				if (ec)
				{
					return;
		...
		void do_write()
		{
			...
			ASIO_NS::async_write(socket_, buffers,
		...
		void shutdown(ASIO_NS::ip::tcp::socket const&)
		{
			error_code ignored_ec;
			socket_.shutdown(ASIO_NS::ip::tcp::socket::shutdown_send, ignored_ec);
		}
#if !DISABLE_ASIO_OPENSSL
		void shutdown(ASIO_NS::ssl::stream<ASIO_NS::ip::tcp::socket> const&)
		{
			//s.async_shutdown()
		}
#endif
	...
	private:
		void delay_write(const void* data, std::size_t size, response::handler_ec_size_t handler)
		{
			...
			if (!reply_.header_buffer_wroted())
			{
				...
				auto self = this->shared_from_this();
				ASIO_NS::async_write(socket_, buffers,
					[self, this, data, size, handler](const error_code& ec, std::size_t /*length*/)
				{
					if (ec)
					{
						handler(ec, 0);
			...
			ASIO_NS::async_write(socket_, ASIO_NS::buffer(data, size), handler);
		}

		void delay_write(std::vector<ASIO_NS::const_buffer> const& buffers, response::handler_ec_size_t handler)
		{
			reset_timer();
			// TODO:合并
			if (!reply_.header_buffer_wroted())
			{
				...
				auto self = this->shared_from_this();
				ASIO_NS::async_write(socket_, buffers,
					[self, this, buffers, handler](const error_code& ec, std::size_t /*length*/)
				{
					if (ec)
					{
						handler(ec, 0);
			...
			ASIO_NS::async_write(socket_, buffers, handler);
		}

		void delay_read(void* data, std::size_t size, response::handler_ec_size_t handler)
		{
			reset_timer();
			ASIO_NS::async_read(socket_, ASIO_NS::buffer(data, size), handler);
		}

		void delay_read_some(void* data, std::size_t size, response::handler_ec_size_t handler)
		{
			reset_timer();
			socket_.async_read_some(ASIO_NS::buffer(data, size), handler);
		}
		...
		void delay_read_chunk(response::handler_strref_intptr_t handler)
		{
			...
			delay_read_some(&chunked_buf_[0], chunked_buf_.size(),
				[this, handler](const error_code& ec, std::size_t length)
			{
				if (ec)
				{
	...
	private:
		...
		ASIO_NS::deadline_timer deadline_;
