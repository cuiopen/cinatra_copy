...
//		response &operator=(response&&) = default;
		...
		using handler_ec_t = std::function<void(error_code const&)>;
		using handler_ec_size_t = std::function<void(error_code const&, std::size_t)>;
		...
		using write_func2_t = std::function<void(std::vector<ASIO_NS::const_buffer> const&, handler_ec_size_t)>;
		...
		public:
			...
			void async_write(std::vector<ASIO_NS::const_buffer> const& buffers, handler_ec_size_t handler) const
			{
		...
		bool to_buffers(std::vector<ASIO_NS::const_buffer>& buffers);
