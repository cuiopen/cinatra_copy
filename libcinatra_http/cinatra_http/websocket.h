...
namespace cinatra
{
	namespace websocket
	{
		...
		struct ws_config_t
		{
			...
			std::function<void(error_code const&)> on_error;
		};

		using async_write_msg_callback_t = std::function<void(error_code const&)>;
		...
		private:
			...
			std::vector<ASIO_NS::const_buffer> format_message(const char *src, size_t length, opcode_t opcode/*, size_t reportedLength, bool compressed*/);
