...
namespace cinatra
{
	namespace websocket
	{
			...
			request const& req, response& rep, boost::string_ref const& client_key, ws_config_t cfg)
		{
			...
			conn->async_write(nullptr, 0, [conn, cfg](error_code const& ec, size_t)
			{
				if (ec)
				{
					if (cfg.on_error)
		...
		void websocket_connection::start()
		{
			...
			conn_->async_read_some(buffer_.data() + LONG_MESSAGE_HEADER, buffer_.size() - LONG_MESSAGE_HEADER, [self, this](error_code const& ec, std::size_t length)
			{
				if (ec)
				{
					if (cfg_.on_error)
		...
		void websocket_connection::async_send_msg(const char* data, std::size_t length, opcode_t opCode, async_write_msg_callback_t handler)
		{
			...
			conn_->async_write(buffers, [handler](error_code const& ec, std::size_t)
			{
				handler(ec);
			});
		}
...
// 			char close_payload[MAX_CLOSE_PAYLOAD + 2];
			...
			auto self = this->shared_from_this();
			async_send_msg(close_payload.get(), close_payload_length, opcode_t::CLOSE, [close_payload, self, this](error_code const& ec)
			{
				if (ec)
				{
					conn_->close();
				...
				{
					conn_->shutdown([](error_code const&) {});
				}
			});
		}
...
// 						}
			...
			else
			{
				...
				if (!remaining_bytes && fin)
				{
					...
					else
					{
						...
						{
							auto self = this->shared_from_this();
							async_send_msg(control_buffer, opcode_t::PONG, [self](error_code const&) {});
							if (cfg_.on_ping)
							{
		...
		std::vector<ASIO_NS::const_buffer> websocket_connection::format_message(
			const char *src, size_t length, opcode_t opCode/*, size_t reportedLength, bool compressed*/)
		{
			...
			return{ ASIO_NS::buffer(msg_header_, header_length), ASIO_NS::buffer(src, length) };
		}
